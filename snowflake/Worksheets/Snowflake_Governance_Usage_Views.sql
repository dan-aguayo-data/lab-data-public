show grants role GOVERNANCE_CES; 

SELECT * FROM MODEL_INFO;

GRANT ALL PRIVILEGES ON SCHEMA CES_GOVERNANCE.TEST TO ROLE GOVERNANCE_CES; 
GRANT ALL PRIVILEGES ON VIEWS CES_GOVERNANCE.TEST TO ROLE GOVERNANCE_CES; 
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA CES_GOVERNANCE.TEST TO ROLE GOVERNANCE_CES;
GRANT ALL PRIVILEGES ON FUTURE TABLES IN SCHEMA CES_GOVERNANCE.TEST TO ROLE GOVERNANCE_CES;



GRANT MONITOR ON ACCOUNT UR45154  TO ROLE GOVERNANCE_CES;
GRANT MONITOR ON ACCOUNT TO ROLE GOVERNANCE_CES;
GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE GOVERNANCE_CES; 
...
CREATE SCHEMA CES_GOVERNANCE.SF_USAGE_METRICS ;
...
;
DROP VIEW CES_GOVERNANCE.SF_USAGE_METRICS.DATABASE_USAGE_L6M;

CREATE or replace VIEW CES_GOVERNANCE.SF_USAGE_METRICS.DATABASE_USAGE_LM AS
...
SELECT * FROM SNOWFLAKE.ACCOUNT_USAGE.DATABASES;
    ...
    DROP VIEW CES_GOVERNANCE.SF_USAGE_METRICS.DATABASE_USAGE;


CREATE OR REPLACE VIEW CES_GOVERNANCE.SF_USAGE_METRICS.DIM_DATE AS

SELECT * FROM COMMON_PROD.DWH.REF_DATE ;


CREATE OR REPLACE VIEW CES_GOVERNANCE.SF_USAGE_METRICS.STAGES_ACTIVE AS

select * from snowflake.account_usage.stages where deleted is null ; 


create or replace view CES_GOVERNANCE.SF_USAGE_METRICS.TABLES_ACTIVE(
	ID,
	TABLE_NAME,
	TABLE_SCHEMA_ID,
	TABLE_SCHEMA,
...
CREATE OR REPLACE VIEW CES_GOVERNANCE.SF_USAGE_METRICS.STORAGE_USAGE AS
...
SELECT DISTINCT ID,TABLE_NAME, TABLE_SCHEMA_ID, TABLE_SCHEMA, TABLE_CATALOG_ID, TABLE_CATALOG  FROM CES_GOVERNANCE.SF_USAGE_METRICS.TABLES_ACTIVE ; 

CREATE OR REPLACE VIEW DIM_WAREHOUSE AS
SELECT DISTINCT WAREHOUSE_ID, WAREHOUSE_NAME FROM CES_GOVERNANCE.SF_USAGE_METRICS.WAREHOUSE_CLOUD_CREDITS ; 
...
SELECT * FROM        CES_GOVERNANCE.SF_USAGE_METRICS.DATABASE_USAGE_LM;



SELECT * FROM CES_GOVERNANCE.TEST.COLLECTIONS_LINKAGES;
SELECT * FROM CES_GOVERNANCE.TEST.PROPERTIES;


SHOW STAGES;


GRANT ROLE GOVERNANCE_CES TO USER "DEAN.NGUYEN@COEXSERVICES.COM.AU"
